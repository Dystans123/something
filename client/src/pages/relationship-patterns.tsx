...
import { RelationshipPatternAnswer, calculateRelationshipPatternResult, getRelationshipPatternProgress, canRelationshipPatternProceed } from "@/lib/relationship-pattern-logic";
...
export default function RelationshipPatterns() {
  ...
  const selectOption = (optionIndex: number) => {
    ...
    setTimeout(() => {
      ...
      if (nextQuestionIndex < relationshipPatternQuestions.length) {
        setCurrentQuestionIndex(nextQuestionIndex);
        window.scrollTo({ top: 0, behavior: 'smooth' });
      } else {
        // Test complete, go to results
        const result = calculateRelationshipPatternResult([...newAnswers]);
        setLocation(`/relationship-pattern-results?pattern=${result.dominantPattern}`);
      }
    }, 600);
  };
  ...
  const nextQuestion = () => {
    if (currentQuestionIndex < relationshipPatternQuestions.length - 1) {
      ...
      // Test complete, go to results
      const result = calculateRelationshipPatternResult(answers);
      setLocation(`/relationship-pattern-results?pattern=${result.dominantPattern}`);
    }
  };
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="text-center">
              ...
              <div className="text-sm text-[hsl(var(--metallic-silver))]">
                Question {currentQuestionIndex + 1} of {relationshipPatternQuestions.length}
              </div>
            </div>
      ...
      <div className="relative z-10 container mx-auto px-6 py-8 max-w-4xl">
        ...
        <AnimatePresence mode="wait">
          ...
          >
            ...
            >
              <span className="inline-block px-4 py-2 bg-gradient-to-r from-emerald-500/20 to-teal-400/20 text-emerald-300 rounded-full text-sm font-medium border border-emerald-400/30">
